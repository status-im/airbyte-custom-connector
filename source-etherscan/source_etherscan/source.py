...
def get_block_by_timestamp(api_key: str, chain_id: str, timestamp: str) -> str:
    res = requests.get("https://api.etherscan.io/v2/api?chainid={chain_id}&module=block&action=getblocknobytime&timestamp={timestamp}&closest=before&apikey={api_key}")
...
class InternalTransaction(EtherscanStream):
    ...
    def parse_response(self, response: requests.Response, stream_slice: Mapping[str, any] = None, **kwargs) -> Iterable[Mapping]:
        ...
        if "status" not in res or res.get("status") != "1":
            raise AirbyteTracedException(
                message="Error when calling the API to get the internal trx",
                internal_message=f"Internal trx fetching in error {res}",
                failure_type=FailureType.config_error
            )
    ...
    def parse_response(self, response: requests.Response, stream_slice: Mapping[str, any] = None, **kwargs) -> Iterable[Mapping]:
        ...
        for trx in res.get("result"):
            yield {
                ...
                "to": trx.get("to"),
                "from": trx.get("from"),
                "contactAddress": trx.get("contactAddress"),
...
class SourceEtherscan(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        if 'tokens' in config:
            streams.append(TokenBalance([Token(name=t.get("name"), address=t.get("address")) for t in config["tokens"]],**default_args))
